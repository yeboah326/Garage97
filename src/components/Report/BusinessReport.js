...
const BusinessReport = () => {
  ...
  const [business_report_summary, setBusinessReportSummary] = useState({
    total_sales_made: 0,
  ...
  const fetchBusinessReport = async () => {
    const response = await fetch(
      `https://sima-backend.herokuapp.com/business/${business_id}/report`,
      {
        method: "GET",
        headers: {
        ...
        <div className="business-report-summary">
          ...
          <div className="business-summaries business-summary ">
            <SummaryCard
              ...
              value={`GHC ${business_report_summary.total_sales_made}`}
            />
            <SummaryCard
              title="Total Stock Purchased"
              value={`GHC ${business_report_summary.total_stock_purchased}`}
            />
            <SummaryCard
              title="Profit/Loss"
              value={`GHC ${business_report_summary.total_profit_or_loss}`}
            />
            <SummaryCard
              title="Total Products Bought"
              value={business_report_summary.total_products_bought}
            />
            <SummaryCard
              title="Total Products Sold"
              value={business_report_summary.total_products_sold}
            />
            <SummaryCard
              title="Products Remaining"
              value={business_report_summary.total_products_remaining}
            />
          </div>
        </div>
            ...
            return (
              <>
                ...
                <div className="product-summaries product-summary">
                  <SummaryCard
                    title="Total Sales Made"
                    value={`GHC ${product_overview.product_sales}`}
                  />
                  <SummaryCard
                    title="Total Stock Purchased"
                    value={`GHC ${product_overview.product_stock}`}
                  />
                  <SummaryCard
                    title="Profit/Loss"
                    value={`GHC ${product_overview.product_profit_loss}`}
                  />
                  <SummaryCard
                    title="Total Products Bought"
                    value={product_overview.products_total_bought}
                  />
                  <SummaryCard
                    title="Total Products Sold"
                    value={product_overview.products_total_sold}
                  />
                  <SummaryCard
                    title="Products Remaining"
                    value={product_overview.products_total_remaining}
                  />
                </div>
              </>
            );
